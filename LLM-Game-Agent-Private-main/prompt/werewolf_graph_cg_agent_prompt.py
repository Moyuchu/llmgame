...
# encoding: utf-8 
# @author: yihuai lan
# @fileName: werewolf_graph_cg_agent.py 
# @date: 2024/4/2 14:48 
#
# describe:
#
...
generate_answer_prompt = """Now its the {}. Given the game rules and conversations above, assuming you are {}, the {}, for questions: {} Generate the correct answer based on the context. If there is not direct answer, you should think and generate the answer based on the context. No need to give options. The answer should in first person using no more than 2 sentences and without any analysis and item numbers."""
...
graph_generate_prompt = """You currently assume the {name} within an Werewolf game, your role is {role} and the game has progressed to the {phase}.
The current question from the host is: {question}, represented with Q.
There are also some intermediate reflection questions before answering the host's question, represented with Q1-Q{n}.
{sub_questions}

Q should depend on the answers to {n} questions but n questions should not depend on Q. There may also be dependencies among {n} questions. 
Please represent the dependencies between questions using a directed graph (using markdown syntax).
...
reasoning_prompt = """You currently assume the {name} within an Werewolf game, your role is {role} and the game has progressed to the {phase}. You are provided some important statements from players and the host and some question-answer pairs. You need to reflect on the question based on the information of summary and question-answer pairs.
...
base_question_prompt = """You currently assume the {name} within an Werewolf game, your role is {role} and the game has progressed to the {phase}.
Current question Q is that {question}
Here are some other question (labeled with Q1~Q{n}) has been answered, you need to choose which questions is helpful to answer question Q.
{base_questions}
ONLY output the labels before the questions.
"""

next_question_prompt = """You currently assume the {name} within an Werewolf game, your role is {role} and the game has progressed to the {phase}.
There are some questions (labeled with Q1~Q{n}) waiting to answer.
{questions}
And there are some answered questions and corresponding answers.
{pairs}

You need to choose one question from Q1~Q{n}, which the question mostly can be answer with the provided answered questions.

ONLY output the label of the question you choose.
"""
