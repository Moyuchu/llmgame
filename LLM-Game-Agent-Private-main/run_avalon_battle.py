...
from src.agents import ChatGPT_SAPARAgent_ForAvalon, ChatGPT_CGAgent
...
def run_game(game_output_dir: str, camp, game_idx):
    ...
    for i in range(player_nums):
        ...
        if roles[i] in camp_role:
            ...
            player_args.append(
                (
                    ChatGPT_SAPARAgent_ForAvalon,
                    {"name": name, "role": role, "role_intro": role_introduction[roles[i].lower()],
                     ...
                     "action_prompt": action_prompt, "response_prompt": response_prompt, "model": model,
                     "tokenizer": tokenizer, "temperature": 0.3,
    ...
    for player_i, agent_i in game.players.items():
        if isinstance(agent_i, (ChatGPT_SAPARAgent_ForAvalon, ChatGPT_CGAgent)):
            agent_i.reflection(
                player_mapping,
